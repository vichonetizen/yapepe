...
def main():
    ...
    print("\n[2b] _is_secret — términos en español y casos comunes (palabra completa vs substring)")
    _ok(device._is_secret(Path("/tmp/contraseñas.txt")), "'contraseñas' se marca como secreto")
    _ok(device._is_secret(Path("/tmp/contrasena_wifi.txt")), "'contrasena' (sin ñ) se marca como secreto")
    _ok(device._is_secret(Path("/tmp/clave.txt")), "'clave' (palabra completa) se marca como secreto")
    _ok(device._is_secret(Path("/tmp/claves_banco.txt")), "'claves' separada por '_' se marca como secreto")
    _ok(device._is_secret(Path("/tmp/secretos.md")), "'secretos' se marca como secreto")
    _ok(device._is_secret(Path("/tmp/credenciales.json")), "'credenciales' se marca como secreto")
    _ok(device._is_secret(Path("/tmp/token.json")), "token.json se marca como secreto")
    _ok(device._is_secret(Path("/tmp/mi-token-github.txt")), "'token' separada por '-' se marca como secreto")
    _ok(device._is_secret(Path("/tmp/ssh_key.txt")), "'key' (palabra completa) se marca como secreto")
    _ok(device._is_secret(Path("/tmp/apikeys.txt")), "'apikey' (substring) se marca como secreto")
    _ok(device._is_secret(Path("/tmp/serviceaccount.json")), "'serviceaccount' se marca como secreto")
    _ok(device._is_secret(Path("/tmp/proyecto-firebase-adminsdk-ab12.json")), "'adminsdk' se marca como secreto")

    print("\n[2c] _is_secret — falsos positivos razonables NO se bloquean")
    ...
    _ok(not device._is_secret(Path("/tmp/tokenizer.py")), "tokenizer.py NO se marca ('token' no es palabra completa)")
    _ok(not device._is_secret(Path("/tmp/monkey.py")), "monkey.py NO se marca ('key' no es palabra completa)")
    _ok(not device._is_secret(Path("/tmp/keyboard.md")), "keyboard.md NO se marca como secreto")
    _ok(not device._is_secret(Path("/tmp/clavel.jpg")), "clavel.jpg NO se marca ('clave' no es palabra completa)")
    _ok(not device._is_secret(Path("/tmp/enclave.md")), "enclave.md NO se marca como secreto")
